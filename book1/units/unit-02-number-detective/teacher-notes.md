...
- **Lesson 4 — while-loop + debugging (60–90 min).**
  ...
  30 min: the `while` ladder (loop until a typed sentinel → loop until the guess matches → add an `if` hint inside → the full random game). The typed rungs are teacher-run (they wait for typing); the scripted-player twin runs by itself and shows three trips with both hints. Every rung is counter-free by design.
  ...
  **Algorithm Extension** section — name the pattern in class from the lesson Spotlight and run Exercise 8
  ...
  worked-example ladder (comparison rungs → a computer-guesser `while` rung (trip count can change each run) →
  a scripted-player twin → the display-only interactive game); like Exercise 8, that ladder is time-permitting/homework enrichment, not required in-class.
  Tip: run the computer-guesser rung several times and have students tally the trips on paper.
  ...
  Route the Pattern Spotlight identify-prompts in project-01 Milestone 1, unit-07 Exercise 13, and unit-10 Exercise 14 to discussion or homework.
