...
- **Lesson 1 — accumulator, logical-ops (60–90 min).**
  ...
  Rest: the full opening round + exercises 1–2. Read the two Pattern Spotlights (running total, counting by condition) from the closing **Algorithm Extension** lesson section here, while the accumulator is fresh; their exercises now live in the end-of-notebook **Algorithm Extension** (Exercises 11–12) and are routed as enrichment — see the Algorithm Extension note below.
