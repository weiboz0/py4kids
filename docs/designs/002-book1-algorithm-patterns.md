...
| `count-by-condition` | Counting by condition (count) | u04 | keep a counter; check each item once, bump it when it matches | u06 count-matches (**new**), u07 Ex11 above-threshold (reuse), u08 Ex5/7 **tally-by-key** (reuse; the *count-per-group* variation, named in the catalog) |
...
| `transform-each` | Do the same to each (map) | u06 | apply the *same step* to every item, producing a **new sequence** | u07 Ex12 (reuse), u08 Ex10 translate-each (reuse), u09 Ex3 line→int (reuse) |
