...
|----|------|------|------|------|
| `running-total` | Running total (accumulate) | u04 | the program needs a *named box* holding the total-so-far, updated each step | u05 Ex11 `total_card_borders` (reuse), u07 Ex10 average (reuse), u09 read-and-sum (**new**) |
| `count-by-condition` | Counting by condition (count) | u04 | keep a counter; check each item once, bump it when it matches | u06 count-matches (**new**), u07 Ex11 above-threshold (reuse), u08 Ex12 **tally-by-key** (reuse; the *count-per-group* variation, named in the catalog) |
| `find-extreme` | Find the best (max / argmax) | u07 | keep the *best so far* while scanning — and remember *who*, not just the number | home = **champion-by-name** (parallel names/scores, keep `best_name`+`best_score`, **new**), u08 Ex13 most-common `best_word`/`best_count` (reuse), u09 best-so-far replacing `max()` (**new**), u10 Ex13 happiest-pet (**promote** — add `best_pet`, not just the number) |
...
| `transform-each` | Do the same to each (map) | u06 | apply the *same step* to every item, producing a **new sequence** | u07 Ex12 (reuse), u08 Ex14 translate-each (reuse), u09 Ex12 line→int (reuse) |
...
| `sentinel-loop` | Repeat until done (sentinel loop) | u02 | *you* know when you're done; the program must ask "done yet?" every pass | project-01 M1 `while choice != "q"` (reuse), u07 Ex13 while (reuse), u10 Ex14 `while happiness<10` (**promote**) — 3 pre-capstone entries. **u04 Ex8 excluded** (`while attempts<3` is a counter-bounded retry, not a sentinel — see the embodiment definition); Ex10's `while stops<3` likewise excluded. |
