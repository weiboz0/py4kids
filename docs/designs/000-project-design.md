...
- **Plan-review gate (4-way):** `[self]` (active session, inline),
  `[sol]` (codex:codex-rescue, GPT-6-sol, fresh read-only),
...
- **Errata:** post-merge content bugs get 2-way diagnosis (Claude inline + Codex GPT-6-sol,
...
| SOLUTIONS to exercises + checkpoints | `codex:codex-rescue` (GPT-6-sol) — separate fresh session, never reads statement outlines; cross-model verification lives in the gates |
