...
| **hybrid — file-read + read the query** (`input()` real-form for the query) | Ex14 find-setting-stop-early — file supplies the records (satisfied), but `target_name` is the QUERY (≈ Ex23's target) → read it: `target_name = input("Which setting? ")` + the unchanged file scan | `**The real program**` block (file read + `input()` target) + `**Real version:**` cue |
...
(non-enrichment) score lists and grow each <4-element one to **≥4 (target 5–6)**, updating the executable
twins' asserts + any printed outputs + the paired real-forms in lockstep:
...
Already-≥4 core lists need not grow. **Enrichment Algorithm-Extension drills (Ex12–Ex23) framed as "small fixed
data" KEEP their small lists** (§3 build-up-rungs/enrichment exemption); their real-forms carry realism via the
arbitrary-`n` read. Growth governs SOURCE lists, not computed-result literals.
...
- `ast.parse` + piped-run every real-form (1 lesson no-exec + 15 solutions markdown); computed-output lines ==
  ...
  time — their piped-run harness prepends Ex6's helper defs and pre-writes the expected file (Ex7: `[300,450,725]`;
  Ex25: `[300,450,725,1350]`) so parity is checked honestly.
...
- Any Book-1 entry other than u09. No new design class (user chose "lighter"); the file-is-real-for-reads
  ...
  <4-element core score lists IS in scope** (see "Data growth"); enrichment small-data drills untouched; no rename.
...
- → [FIXED] [sol]#3 **§3 data growth** — new "## Data growth (§3)" section: grow the <4-element core SOURCE
  score lists (lesson loop-write, Ex1, Ex6, Ex9) to ≥4; update asserts/outputs/real-forms; enrichment (Ex12-23)
  "small fixed data" drills stay small.
