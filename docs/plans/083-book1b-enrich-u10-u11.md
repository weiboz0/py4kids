...
- **Values:** every new fixture below is grepped against shipped Book 1b content (Phase E); the
  author already replaced `banana` (shipped in U09/U11) and the names `Ana`/`Bo` (shipped in U11).

## U10 — Lists

**Rungs (lesson):**
1. **Split the dense first cell.** `u10l003` currently teaches a list literal, a function, `append`
   (before its section) and index 0 at once. Replace it with rungs, each with its own lead-in (the
   first paragraph of `u10l002` stays as rung 1's lead-in):
   ...
   (d) `print(len(scores))`; (e) `scores[1] = 5` / `print(scores)` (change one item — lists can
   change, strings cannot). The old `u10l003`/`u10l004` pair moves, rewritten without `append`
...
3. **Slices on lists:** one rung `print(scores[1:3])` then `print(scores[::-1])` (a new reversed
   list; the original is unchanged), placed after the position loop (`u10l011`–`u10l013`).
4. **New section at the end of Lesson 1: "Read several numbers on one line"** — rungs:
   `line = "4 9 2"` / `parts = line.split()` / `print(parts)` (a list of strings); `print(int(parts[0])
   + int(parts[1]))`; then a loop that appends `int(part)` for every piece into `numbers`.
...
5. **New section before `u10l046` (put it together): "Lists inside lists: a grid"** — rungs:
   `grid = [[1, 0, 1], [0, 1, 0]]` / `print(grid[1])`; `print(grid[1][0])` (row, then column);
...
`insert`/`remove`/`pop`/`index` are now taught; `count` stays untaught).
...
| Second Largest | C | `second_largest([7, 19, 4, 19, 12])` → 12 (the largest value strictly below `max`: seed `best = min(numbers)`, then keep any value that is `< top` and `> best`), `([5, 3])` → 3; lists hold at least two distinct values; real program prints `Second largest: 12` (f-string; purpose comment required) | searching |
...
| Median | MP | `median([9, 2, 7, 4, 5])` → 5; `median([8, 3, 6, 1])` → 4.5 (`sorted`, middle index `len // 2`, even length averages the two middles); real prints `Median: 5` | statistics |
...
| Queue at the Counter | MP | `after_rush(["Ada", "Ben", "Cal"])` → append `"Dee"`, serve with `pop(0)`, `insert(0, "Eve")`, `remove("Cal")` → `['Eve', 'Ben', 'Dee']`; real reads the three names on one line | simulation, list methods |
| Find the Seat | MP | `seat_of(["Kim", "Lu", "Max"], "Lu")` → 1, `(…, "Zed")` → -1 (`.index` guarded by `in`) | searching |
| Bar Chart | MP | `bar_chart(["Mon", "Tue", "Wed"], [3, 5, 2])` → `"Mon ###\nTue #####\nWed ##"` (index loop over two parallel lists) | ASCII art, data report |
...
| Long Words | MP | `long_words(["kite", "umbrella", "sky", "lantern"], 5)` → `['umbrella', 'lantern']` (`len(word) >= minimum`) | filtering, text |
| Common Elements | MP | `common([3, 8, 5, 12], [5, 3, 7])` → `[3, 5]` (order of the first list) | filtering |
| Class Average | MP | `average_of([12, 15, 9, 20])` → 14.0 with a hand-written total loop (no `sum`); real program reads `n` then `n` lines and prints `Average: 14.0` | statistics |
| Challenge: Vertical Bar Chart | S | `vertical_bars([3, 1, 2])` → `"#..\n#.#\n###"` (levels from `max` down to 1; `#` when the value reaches the level, else `.`) | ASCII art |
...
| Challenge: Statistics Report | S | `stats_report([6, 2, 9, 2, 5])` → `"mean 4.8 median 5 mode 2"` — calls the student's own `median` and `mode` helpers (composition) | statistics |
...
3. **In "Build a frequency map" (after `u11l033`):** a rung that tallies the letters of one word
   (`for ch in "cocoa":`) — the tally over a string, before the `get` shortcut.
...
**Shipped text to rewrite:** `u11e001` intro (new partition); teacher-notes lines naming the Core/
Challenge split and pacing.
...
| Letter Tally Chart | C | `tally_chart("cocoa")` → `"c ##\no ##\na #"` (rows in first-appearance order — dictionaries keep insertion order) | ASCII art, tallies |
| Invert a Dictionary | MP | `invert({"north": "N", "south": "S", "east": "E"})` → `{'N': 'north', 'S': 'south', 'E': 'east'}` (values are unique) | encoding |
...
| Word Translator | MP | `translate("the cat sat", {"cat": "gato", "the": "el"})` → `"el gato sat"` (`get(word, word)` keeps unknown words) | encoding, text |
...
| Alphabetical Keys | MP | `sorted_keys({"quince": 4, "fig": 9, "apple": 2})` → `['apple', 'fig', 'quince']` (append each key, then `.sort()`) | sorting |
...
- **U10 (36):** 1–7 existing core · 8 Numbers on One Line · 9 Second Largest · 10 Rotate a List ·
  11–29 MP: Median, Mode, Remove Duplicates, Reverse in Place, Top Three, Queue at the Counter, Find the
  Seat, Bar Chart, Grid Printer, Tic-Tac-Toe Winner, Magic Square Check, Running Maximum, Fix the Index
  Error, Fix the Missing Value, Smallest Gap, Is It Sorted?, Long Words, Common Elements, Class
  Average · 30–36 Challenges: Keep Approved Values (was 8), Longest Word Position (was 9), Vertical Bar
...
No real version: U10 Fix the Index Error, Fix the Missing Value (U10 Ex 4 "Repair the In-Place Sorter" is
...
- **U10 practices:** `builtin-functions` ≥10; `in-operator` 5 (Keep Approved Values, Remove
  ...
  Life Step); `loop-counter` ≥3; `f-string` 3 (real programs of Second Largest, Median, Class Average);
  `float-type` 3 (Median, Class Average, Statistics Report); `comment` 3 (Ex 8–10); `int-type`,
  `naming` ≥3.
...
- **U11 practices** (reps pinned in the specs): `list-literal`/`list-append` ≥5; `list-sort` 3 (Ex 7
  ...
  Percentages); `f-string` 3 (Word Winner Report, Report Card rows `f"{name} {average}"`, Price Lookup
  Total's real program `f"Total: {total}"`); `builtin-functions` ≥3; `comment` 3 (purpose comments
...
Expected: U10 `practices` += `input`, `type-conversion`, `string-methods` (`split`, `join`),
`string-index` (grid strings), `string-concat`, `nested-loops`, `while-loop` (Reverse in Place),
`error-messages` (the two repairs), `logical-ops` (Minesweeper bounds), `elif-else`, `string-slice`
(list slices scan as slices). U11 `practices` += `input`, `type-conversion`, `float-type` (Vote
Percentages, Report Card), `error-messages` (Fix the KeyError), `logical-ops`/`elif-else` only if the
honesty scan finds them. Any delta the scan does not confirm is not added.
