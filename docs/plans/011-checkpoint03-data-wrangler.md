...
`comparison`/`boolean` (conditionals + membership predicates), `for-loop` (list/dict loops),
...
- Coverage-map amendment (EXACTLY this, pre-audited + scanner-validated green):
  ...
  - append to `checkpoint-03-data-wrangler.practices` — `print, variable, f-string,
...
term-3 headline concepts + amended print/variable/f-string/string-literal/if-statement/boolean/
...
- **Q6 — a price book (dict-literal, dict-access, in-operator, if-statement, boolean):** build
  `prices = {"apple": 3, "pear": 2, "plum": 4}`; look up `prices["pear"]`; use `.get("fig", 0)` for
  a missing key; print whether `"apple" in prices`; an `if`/`else` prints "sold out" vs the price.
...
- Teacher notes: SIX headings incl. `## Grading` — per-question point allocation, what full credit
  looks like, and common partial-credit cases; 30-min pacing (checkpoint is 0.5 lesson); common
  mistakes (off-by-one slice bounds; `.sort()` returns None; KeyError vs `.get`; counting the dict
  instead of the word list); differentiation (a struggling student may skip Q5/Q8).
